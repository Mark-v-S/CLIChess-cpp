...
void playerMove1(char** space, char* charsp, char* charspe)
{
    ...
    do {
        ...
        {
            case '|':
            moved = Pawn(space, row, slot, charsp[0], charspe, charsp[6], 6, -1);
            break;
            case '+':
            moved = Rock(space, row, slot, charsp[1], charspe);
            break;
            case 'r':
            moved = Knight(space, row, slot, charsp[2], charspe);
            break;
            case 'x':
            moved = Bishop(space, row, slot, charsp[3], charspe);
            break;
            case '*':
            moved = Quin(space, row, slot, charsp[4], charspe);
            break;
            case '#':
            moved = King(space, row, slot, charsp[5], charspe);
            break;
        }
    } while (moved == false);
}
...
bool Bishop(char** space, int row, int slot, char piece, char* pos)
{
    ...
    if (slot < 7 && row < 7)
    {
        moved = moveloop(slot, row, 1, 1, moveslot, moverow, space, pos, piece);
    }
    if (slot > 0 && row > 0)
    {
        moved = moveloop(slot, row, -1, -1, moveslot, moverow, space, pos, piece);
